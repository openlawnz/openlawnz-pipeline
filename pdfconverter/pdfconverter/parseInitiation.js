const { parse2, parse3, parse4, mergeData } = require("./parseUtility.js");
module.exports = (data) => {
    let initiationNames = [];
    ...
    const initiationPattern1 = [
        ...
        /(.*)\s*(First )?\s*(Appellants?|Applicants?|Plaintiffs?)/mi,
        /(THE QUEEN)(v)\s{1,7}/mi
    ];
    ...
    const initiationPattern2 = [
        ...
        //2004-2005
        /(BETWEEN)\s*([a-zA-Z0-9\u00C0-\u02AB\s'\(\),’]*)\s*(First )?\s*(Appellants?|Applicants?|Plaintiffs?)/gmi
    ];
    let result2 = parse3(data, initiationPattern2, 2);

    mergeData(initiationNames, result2);

    //Get the applicant/appelant/plaintiff name between number
    const initiationPattern3 = [
        /(First )?(Appellants?|Applicants?|Plaintiffs?)\s*(AND)?\s*([a-zA-Z0-9\u00C0-\u02AB\s'\(\),]*)\s{2}(Second )?\s*(Appellants?|Applicants?|Plaintiffs?)/gmi,
        /(Second )?(Appellants?|Applicants?|Plaintiffs?)\s*(AND)?\s*([a-zA-Z0-9\u00C0-\u02AB\s'\(\),]*)\s{2}(Third )?\s*(Appellants?|Applicants?|Plaintiffs?)/gmi,
        /(Third )?(Appellants?|Applicants?|Plaintiffs?)\s*(AND)?\s*([a-zA-Z0-9\u00C0-\u02AB\s'\(\),]*)\s{2}(Fourth )?\s*(Appellants?|Applicants?|Plaintiffs?)/gmi
    ];
    let result3 = parse3(data, initiationPattern3, 4);
